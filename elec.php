...
<link rel="stylesheet" href="layout/css/bootstrap.min.css">
  ...
  <body>
	  <?php
	   ...
	   include "include/DBClass.php";
		...
		$month=$_POST['month'];
		 ...
		 $row=$db->getRow("select * from users , invoice where social_id=$ssid and service='e' and month=$month and U_id=(select ID from users where social_id=$ssid) ");
		$rcount=$db->getCount("select * from users , invoice where social_id=$ssid and service='e' and month=$month and U_id=(select ID from users where social_id=$ssid) ");
	    $m=$row['Month'];
		  $fn=$row['FullName'] ;
		  $g= $row['Govern'] ;
		 $c= $row['Center'];
	  ...
	  if($rcount>0){
		  ...
		  echo'
	  <h3 class="text-center">فاتورة كهرباء</h3>
